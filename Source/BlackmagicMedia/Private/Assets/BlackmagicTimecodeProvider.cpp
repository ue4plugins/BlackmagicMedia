...
namespace BlackmagicTimecodeProviderHelpers
{
	...
	public:
		FEventCallback(const BlackmagicDesign::FChannelInfo& InChannelInfo, const FFrameRate& InFrameRate)
			...
			, bIsDropFormatTimecodeSupported(false)
			, State(ETimecodeProviderSynchronizationState::Closed)
			, FrameRate(InFrameRate)
			, bHasWarnedMissingTimecode(false)
		{
			bIsDropFormatTimecodeSupported = FTimecode::IsDropFormatTimecodeSupported(InFrameRate);
		}
	...
	private:
		...
		virtual void OnFrameReceived(const BlackmagicDesign::IInputEventCallback::FFrameReceivedInfo& InFrameInfo) override
		{
			...
			if (InFrameInfo.bHaveTimecode)
			{
				...
				Timecode = FTimecode(InFrameInfo.Timecode.Hours, InFrameInfo.Timecode.Minutes, InFrameInfo.Timecode.Seconds, InFrameInfo.Timecode.Frames, bIsDropFormatTimecodeSupported);
			}
	...
	private:
		...
		FTimecode Timecode;
		bool bIsDropFormatTimecodeSupported;
...
{
	return EventCallback && EventCallback->GetSynchronizationState() == ETimecodeProviderSynchronizationState::Synchronized ? EventCallback->GetTimecode() : FTimecode();
}

FFrameRate UBlackmagicTimecodeProvider::GetFrameRate() const
{
